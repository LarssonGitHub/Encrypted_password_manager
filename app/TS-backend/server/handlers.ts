...
export const deleteData = async (id: string, key: string): Promise < backendResponse > => {
    ...
    if (compiledData.length <= 0) {
        ...
        if (!resetDatabase) throw new Error("<customThrownError::>Couldn't clean the database");
        return createResponse("Successfully deleted record, your database is empty", null, true);
    }
...
export const postData = async (postData: userCredentialObject, key: string): Promise < backendResponse > => {
    if (!postData || Object.values(postData).includes(""))
        throw new Error("<customThrownError::>No data was submitted, or fields were left empty");
