...
export const itemDialog = document.getElementById("item-dialog") as HTMLDialogElement;
export const closeItemButton = document.getElementById("close-item-button") as HTMLButtonElement;
export const deleteItemButton = document.getElementById("delete-item-button") as HTMLButtonElement;
...
export const InsertDataIntoTemplate = (data: userCredentialsArray | void): void => {
	...
	sortedData.forEach((key) => {
		...
		(insertedCloneText.querySelector(".data-storage") as HTMLElement).setAttribute("data-stored-object", JSON.stringify(key));
...
export const appendItemValuesToForm = (): void => {
	const compiledData: userCredentialObject = getDataStoredObject(editItemButton);
